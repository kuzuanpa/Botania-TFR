...
public class ClientProxy extends CommonProxy {
	...
	private void initRenderers() {
		...
		MinecraftForgeClient.registerItemRenderer(Item.getItemFromBlock(ModBlocks.floatingFlower),
				renderFloatingFlower);
		MinecraftForgeClient.registerItemRenderer(Item.getItemFromBlock(ModBlocks.floatingSpecialFlower),
				renderFloatingFlower);
		...
		RenderingRegistry.registerEntityRenderingHandler(EntityEnderAirBottle.class,
				new RenderSnowball(ModItems.manaResource, 15));
		...
		RenderingRegistry.registerBlockHandler(
				ModBlocks.crucibleRenderId2 = RenderingRegistry.getNextAvailableRenderId(), new RenderCrucible());
	...
	public void setToTutorialIfFirstLaunch() {
		if (PersistentVariableHelper.firstLoad)
			GuiLexicon.currentOpenLexicon = new GuiLexiconEntry(LexiconData.welcome,
					new GuiLexiconEntry(LexiconData.tutorial, new GuiLexicon())).setFirstEntry();
	}
	...
	public void setExtraReach(EntityLivingBase entity, float reach) {
		...
		if (entity == player) {
			if (!(mc.playerController instanceof IExtendedPlayerController)) {
				GameType type = ReflectionHelper.getPrivateValue(PlayerControllerMP.class, mc.playerController,
						LibObfuscation.CURRENT_GAME_TYPE);
				NetHandlerPlayClient net = ReflectionHelper.getPrivateValue(PlayerControllerMP.class,
						mc.playerController, LibObfuscation.NET_CLIENT_HANDLER);
			...
			((IExtendedPlayerController) mc.playerController).setReachDistanceExtension(
					Math.max(0, ((IExtendedPlayerController) mc.playerController).getReachDistanceExtension() + reach));
		}
	}
	...
	@Override
	public void lightningFX(World world, Vector3 vectorStart, Vector3 vectorEnd, float ticksPerMeter, long seed,
