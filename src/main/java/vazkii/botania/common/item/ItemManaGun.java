...
public class ItemManaGun extends ItemMod implements IManaUsingItem {
    ...
    public ItemManaGun(boolean isAdvanced) {
        super();
        this.isAdvanced=isAdvanced;
        setMaxStackSize(1);
        setNoRepair();
        if(isAdvanced){
            setMaxDamage(COOLDOWN*2);
            setUnlocalizedName(LibItemNames.MANA_GUN_ADVANCED);
        }
        else {
        ...
        if(par3EntityPlayer.isSneaking() && hasClip(par1ItemStack)) {
            ...
            if(burst != null && ManaItemHandler.requestManaExact(par1ItemStack, par3EntityPlayer, burst.getMana(), true)) {
                if(!par2World.isRemote) {
                    ...
                    par3EntityPlayer.swingItem();
                    par3EntityPlayer.motionX -= burst.motionX * 0.05;
                    par3EntityPlayer.motionY -= burst.motionY * 0.1;
                    par3EntityPlayer.motionZ -= burst.motionZ * 0.05;
                }
                par1ItemStack.setItemDamage(effCd);
