		...
		if(ticksExisted >= 55) {
			...
			if(!worldObj.isRemote) {
				...
				for(EntityPlayer player : players) {
					player.attackEntityFrom(summoner == null ? DamageSource.generic : DamageSource.causeMobDamage(summoner), 60);
					player.addPotionEffect(new PotionEffect(Potion.blindness.id, 30, 0));
					PotionEffect wither = new PotionEffect(Potion.wither.id, 60, 3);
