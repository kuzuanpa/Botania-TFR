		...
		if(target != null) {
			...
			if(targetList.contains(target) && target != null) {
				...
				if(thrower != null) {
					...
					target.attackEntityFrom(player == null ? DamageSource.causeMobDamage(thrower) : DamageSource.causePlayerDamage(player), evil ? 80 : 60);
				} else target.attackEntityFrom(DamageSource.generic, evil ? 80 : 60);
